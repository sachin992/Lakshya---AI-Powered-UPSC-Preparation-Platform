...
#         with day_cols[i]:
#             st.markdown(f'<div style="text-align: center; font-weight: bold; color: #1e40af;">{day}</div>', unsafe_allow_html=True)
...
#                     if is_selected:
#                         if st.button(str(day), key=f"day_{day}", use_container_width=True, type="primary"):
#                             st.session_state.selected_date = st.session_state.selected_date.replace(day=day)
#                     else:
#                         if st.button(str(day), key=f"day_{day}_normal", use_container_width=True):
